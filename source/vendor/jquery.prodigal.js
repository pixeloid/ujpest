;(function($, window, document, undefined){
	var options = {
		...
		tpl : {
			...
			close: '<div class="prodigal_close"><div class="close_button"><div class="close_symbol">&#215;</div></div></div>',
	...
	methods = {
		...
		resize: function(resizeImage){
			...
			options._wrapper.css({ top: opts.spaceFromEdge, left: pagex+opts.spaceFromEdge });
			// eredeti: options._wrapper.css({ top: pagey+opts.spaceFromEdge, left: pagex+opts.spaceFromEdge });
			...
			gallery_left.css({
				...
				width: (options._wrapper.width()-options._wrapper.find('.prodigal_gallery_right').width())-
					(parseInt(gallery_left.css('paddingLeft'))+parseInt(gallery_left.css('paddingRight')))-(leftW)
			});
	...
	imageClickHandler = function(e){
		...
		if($(this).data('prodigal-media-type') !== null && $(this).data('prodigal-media-type') == 'video'){
			...
			$('.prodigal_gallery_left').zoom({ on:'click' });
		}
	},
